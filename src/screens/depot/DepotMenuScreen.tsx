...
export default function DepotMenuScreen() {
  ...
  return (
    <View className="flex-1 bg-white" style={{ paddingTop: insets.top }}>
      ...
      <View className="z-10 bg-white px-4 py-3 shadow-sm">
      ...
      {isMenusLoading ? (
        ...
        <SectionList
          ...
          contentContainerStyle={{
            ...
            paddingBottom: hasActiveCartHere ? 100 : 60,
          }}
          refreshControl={
            <RefreshControl
      ...
      {hasActiveCartHere && (
        <View
          className="absolute bottom-0 left-0 right-0 border-t border-gray-100 bg-white px-4 pt-4 shadow-[0_-10px_15px_-3px_rgba(0,0,0,0.05)]"
          style={{ paddingBottom: Math.max(insets.bottom, 16) }}>
          <TouchableOpacity
            activeOpacity={0.8}
            onPress={() => navigation.navigate('CartScreen')}
            className="w-full flex-row items-center justify-between rounded-2xl bg-bakso-primary p-4 shadow-sm">
